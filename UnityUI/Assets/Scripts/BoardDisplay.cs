...
public class BoardDisplay : MonoBehaviour
{
    ...
    public void UpdateVisuals()
    {
        ...
        for (int i = 0; i < 7; i++) for (int j = 0; j < 7; j++)
        {
            ...
            if (IsOccupied(bitBoard.player1, i, j)) { colour = Color.grey; }
            else if (IsOccupied(bitBoard.player2, i, j)) { colour = Color.orange; }
            else
            {
                if (IsOccupied(vision, i, j)) colour = Color.green;
            ...
            if (j < 6)
            {
                if (IsOccupied(bitBoard.vertical_walls, i, j + 1)) colour = Color.black;
                else
                {
                    colour = Color.white;
                    if (second_cell is (int r2, int c2))
            ...
            if (i < 6)
            {
                if (IsOccupied(bitBoard.horizontal_walls, i + 1, j)) colour = Color.black;
                else
                {
                    colour = Color.white;
                    if (second_cell is (int r2, int c2))
    ...
    public void OnCellClicked(int r, int c)
    {
        ...
        {
            first_cell = (r, c);
            if (!IsOccupied(bitBoard.AllPieces, r, c))
            {
                Turn t = new Turn(Border.NaB, 0, BitBoard7x7.Index(r, c));
                MakeTurn(t);
            }
        }
        else
    ...
    public void OnWallClicked(int r, int c, bool isHorizontal)
    {
        ...
        {
            Border border = DetermineDirection(r2, c2, r, c, isHorizontal);
            Turn t = new Turn(border, BitBoard7x7.Index(r1, c1), BitBoard7x7.Index(r2, c2));
            MakeTurn(t);
        }
        else
        {
    ...
    public bool IsOccupied(ulong bitstring, int i, int j)
    {
        return BitBoard7x7.HasBit(bitstring, BitBoard7x7.Index(i, j));
    }
